...
#include <cmath>
#include <limits>
#include "Mixture.h" //for read_timer()
...
//Forrest's HOG extraction timers
double time_fill_atan_table = 0;
double time_bilinear_interpolation = 0;
double time_gradient_energy = 0;
double time_normalize_features = 0;
double time_truncation_features = 0;
...
pady_(0), interval_(0)
{
	...
	pady_ = pady;
	interval_ = interval;
	levels_.resize(maxScale + 1); //TODO: add timer here.
	
	int i;
#pragma omp parallel for private(i) 
	for (i = 0; i < interval; ++i) {
		double scale = pow(2.0, static_cast<double>(-i) / interval); //TODO: add timer here.
    ...
    printf("    HOG: precompute atan table in %f ms \n", time_fill_atan_table);
    printf("    HOG: bilinear interpolation in %f ms \n", time_bilinear_interpolation);
    printf("    HOG: gradient energy in %f ms \n", time_gradient_energy);
    printf("    HOG: normalize features in %f ms \n", time_normalize_features);
    printf("    HOG: truncation features in %f ms \n", time_truncation_features);
}
...
double start_fill_atan_table = read_timer();
	
	// Fill the atan2 table
#pragma omp critical
	if (ATAN2_TABLE[0][0] == 0) { //TODO: fill this in a separate function, only compute it once.
...
double local_time_fill_atan_table = read_timer() - start_fill_atan_table;
time_fill_atan_table += local_time_fill_atan_table;


double start_bilinear_interpolation = read_timer();
	...
	for (int y = 0; y < height; ++y) {
		...
		for (int x = 0; x < width; ++x) {
            ...
            // in the current loop,'find largest gradient magnitude' takes ~1/3 of the computation time.	
            ...
            // in the current loop, bilinear interpolation takes ~2/3 of the computation time
			if (cellSize == 8)
...
double local_time_bilinear_interpolation = read_timer() - start_bilinear_interpolation;
time_bilinear_interpolation += local_time_bilinear_interpolation;

double start_gradient_energy = read_timer();
...
double local_time_gradient_energy = read_timer() - start_gradient_energy;
time_gradient_energy += local_time_gradient_energy;	

double start_normalize_features = read_timer();
...
double local_time_normalize_features = read_timer() - start_normalize_features;
time_normalize_features += local_time_normalize_features;	

double start_truncation_features = read_timer();
	// Truncation features
	for (int y = 0; y < level.rows(); ++y) {
		for (int x = 0; x < level.cols(); ++x) {
...
double local_time_truncation_features = read_timer() - start_truncation_features;
time_truncation_features += local_time_truncation_features;
